  ...
  begin
    ...
    begin
      ...
      if Copy(TmpS, 2, 1) = '1' then
        ...
        Set_Config(2, false); // для PIC10F20x - WDTE
      if Copy(TmpS, 1, 1) = '1' then
        Set_Config(3, true)
      else
        Set_Config(3, false); // для PIC10F20x - \CP
      ...
      if Copy(TmpS, 4, 1) = '1' then
        ...
        Set_Config(4, false); // для PIC10F20x - GP3/\MCLR
      if Copy(TmpS, 3, 1) = '1' then
        Set_Config(5, true)
      else
        Set_Config(5, false);
    ...
    //Третью треть слова загружать в память не имеет смысла для Baseline архитектуры
